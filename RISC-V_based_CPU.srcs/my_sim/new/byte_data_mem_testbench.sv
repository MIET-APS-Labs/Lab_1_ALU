...
module byte_data_mem_testbench ();
  logic CLK;
  ...
  logic [`MEM_TYPE_LOAD_STORE_BIT-1:0] MEM_SIZE;
  logic [`WORD_LEN-1:0] RD;


  logic [`WORD_LEN-1:0] data_read;

  byte_data_mem #(`WORD_LEN, `MEM_TYPE_LOAD_STORE_BIT, `MEM_DEPTH) dut (
      .clk (CLK),
      .adr (ADR),
      .wd  (WD),
      .we  (WE),
  ...
  task check_write_read_equal(input [`MEM_TYPE_LOAD_STORE_BIT-1:0] task_mem_size);
